...
T* alloc(size_t size) {
    if(global_state.device < 0) {
        ...
        void* memory;
        cudaError_t error = cudaMalloc(&memory, sizeof(T) * size);
        if(error != cudaSuccess) {
...
void copy(T* dst, const T* src, size_t size) {
    if(global_state.device < 0) {
        ...
        // GPU memory allocation through CUDA allocator.
        cudaMemcpyAsync((void*) dst, (void*) src, size * sizeof(T),
...
template<class T>
__global__ void kernel_add(const T* x, const T* y, const int N, T* out) {
...
// Both buffers must resize on the same device.
template<class T>
void reduce(T* dst, T* src, size_t size) {
    if(global_state.device < 0) {
...
void RingAllreduce(const T* data, size_t length, T* output) {
    ...
    for (int i = 0; i < size - 1; i++) {
        ...
        T* segment_send = &(output[segment_ends[send_chunk] -
    ...
    for (size_t i = 0; i < size_t(size - 1); ++i) {
        ...
        T* segment_recv = &(output[segment_ends[recv_chunk] -
                                       segment_sizes[recv_chunk]]);
...
template<class T>
void RingAllgather(const T* data, size_t length, T* output_ptr) {
    // Get MPI size and rank.
    int rank;
    int mpi_error = MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    if(mpi_error != MPI_SUCCESS)
